...
[source,input,subs="+macros,+attributes"]
----
curl "$(oc get routes film-storage-online -n {user}-blue-green --template='http://{{.spec.host}}')/api/v1/info" | json_pp
----
...
[source,input,subs="+macros,+attributes"]
----
curl "$(oc get routes film-storage-offline -n {user}-blue-green --template='http://{{.spec.host}}')/api/v1/info" | json_pp
----
